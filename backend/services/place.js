...
const findOrCreatePlace = async (placeName) => {
    try {
        ...
        if (place) { return { status: 200, message: "PLACE_ALREADY_EXISTS", place }; }
        ...
        if (response.data.length === 0) {throw { status: 404, message: "NO_RESULTS_IN_NOMINATIM" };}
...
const getPlaces = async () => {
    try {
        ...
        if (!places) { throw { status: 404, message: "NO_PLACES_FOUND" }; }
        return places;
    } catch (error) {
